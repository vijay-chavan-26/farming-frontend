...
import API_URL, { get_request, put_request } from "../../utils/ApiRequests";
...
const VehiclesTable = () => {
  ...
  const fetchData = (UsersTableData) => {
    ...
    const updatedData = modifiedData?.map((item,index) => {
      ...
      return {
       ...
       desc: item.desc.length > 15 ? `${item.desc.substring(0, 15)}...` : item.desc,
        ...
        action: <ActionDropdownStatus item={item} onStatusChange={onStatusChange} />,
      };
    });
    setData(updatedData);
    setLoading(false);
  ...
  const onStatusChange = async(item,status) =>{
    try {
      const res = await put_request(`${API_URL}/partner/change-status`,{itemId: item._id,status:status});
      console.log(res)
      if (res) {
        fetchUserData().then((res) => {
          fetchData(res);
